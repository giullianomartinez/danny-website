# Planning - Landing Danny DJ

## Objetivo

Crear una landing page para un DJ y editor de video en Iquique que convierta visitas en contactos por WhatsApp para contratar servicios de musica, visuales en vivo y edicion audiovisual.

## Stack

- Python 3
- Flask
- Jinja templates
- CSS responsive sin framework pesado
- Assets locales dentro del proyecto

## Restriccion de herramientas

Todas las herramientas necesarias fuera de Codex deben ser gratis:

- Python: gratis y open source.
- Flask: gratis y open source.
- pip/venv: incluidos en el ecosistema gratuito de Python.
- Editor recomendado: VS Code o cualquier editor gratuito.
- Navegador: Chrome, Edge, Firefox o similar.
- WhatsApp link: gratis usando `wa.me`.
- Deploy opcional: usar alternativas gratuitas o free tier como Render, PythonAnywhere o Fly.io, revisando sus limites vigentes.

El sitio no debe depender de servicios pagados, plantillas premium, CDNs pagos ni librerias comerciales.

## Audiencia

- Personas organizando matrimonios, cumpleanos, fiestas privadas y eventos corporativos.
- Marcas locales que necesitan musica, registro y contenido para redes.
- Productoras que buscan un perfil con experiencia y criterio audiovisual.

## Propuesta de valor

Un servicio que combina experiencia musical, lectura de pista, puesta en escena y criterio audiovisual para que cada evento suene bien, se vea profesional y deje contenido util para redes.

## Estructura de la landing

1. Hero con nombre, ubicacion y CTA principal.
2. Indicadores de trayectoria para generar confianza.
3. Presentacion del perfil y valor diferencial.
4. Servicios principales:
   - DJ para eventos.
   - Video y aftermovie.
   - Visuales en vivo.
5. Paquetes iniciales para facilitar la cotizacion.
6. Proceso de trabajo paso a paso.
7. Testimonios o prueba social editable.
8. CTA final a WhatsApp.

## Contenido pendiente por reemplazar

- Nombre artistico definitivo.
- Numero real de WhatsApp.
- Precios reales o rangos actualizados.
- Fotografias reales de eventos.
- Videos o reels de muestra.
- Testimonios reales.
- Links a Instagram, YouTube, TikTok u otras redes.

## Fases de trabajo

### Fase 1 - Base funcional

- Crear app Flask.
- Crear template principal.
- Crear CSS responsive.
- Agregar hero visual local.
- Agregar CTA a WhatsApp.
- Verificar carga local.

### Fase 2 - Contenido real

- Reemplazar textos provisionales.
- Ajustar paquetes y precios.
- Agregar telefono real.
- Incorporar imagenes y videos propios.
- Mejorar testimonios con casos reales.

### Fase 3 - Conversion

- Optimizar CTA principal.
- Agregar mensajes prellenados para WhatsApp.
- Crear secciones por tipo de evento si hace falta.
- Revisar mobile first.
- Medir clics en WhatsApp si se agrega analytics gratuito.

### Fase 4 - SEO local

- Optimizar titulo y descripcion.
- Agregar keywords relacionadas con DJ en Iquique.
- Crear contenido para eventos, matrimonios, fiestas y marcas locales.
- Preparar metadatos sociales.

### Fase 5 - Publicacion

- Elegir hosting gratuito o free tier.
- Configurar variables y entorno si aplica.
- Probar dominio o URL publica.
- Revisar performance y responsive final.

## Checklist de calidad

- El sitio carga localmente sin errores.
- No hay dependencias pagadas.
- La UI es responsive en desktop y movil.
- El hero comunica rapidamente el servicio.
- Los CTA apuntan a WhatsApp.
- Los textos pueden editarse facilmente desde `app.py`.
- Los assets usados estan dentro del proyecto.
- El planning queda documentado en Markdown.
